  ...
  for (let i = 0; i < dadosFilmes.results.length; i++) {
    ...
    dadosHTML += `
        <div class="card col-4">
            <img src="https://image.tmdb.org/t/p/w500${filme.poster_path}" class="card-img-top"  width="200px" alt="Card image cap">
            <div class="card-body">
                <h5 class="card-title">${filme.title}</h5>
                <p class="card-text">${filme.overview}</p>
                <a href="detalhes.html?id=${filme.id}" class="btn btn-primary">Mais detalhes</a>
            </div>
        </div>
        `;
  }
...
const init = () => {
  ...
  let url =
    "http://api.themoviedb.org/3/discover/movie?api_key=e9f3a6f2ce3a6b0e8d89714153057110&sort_by=popularity.desc&page=1&with_original_language=ja";
